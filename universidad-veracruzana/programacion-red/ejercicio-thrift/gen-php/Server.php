...
$loader->registerDefinition('calculator', $GEN_DIR);
$loader->register();
 ...
 * THttpClient in any language that supports it.  The PHP calculator client
...
class CalculatorHandler implements \calculator\CalculatorIf {
  protected $log = array();
  ...
  public function calculate($logid, \calculator\Work $w) {
    ...
    switch ($w->op) {
      ...
      case \calculator\Operation::DIVIDE:
        if ($w->num2 == 0) {
          $io = new \calculator\InvalidOperation();
          $io->whatOp = $w->op;
...
$processor = new \calculator\CalculatorProcessor($handler);
